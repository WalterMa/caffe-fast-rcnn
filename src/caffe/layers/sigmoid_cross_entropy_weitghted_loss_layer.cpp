...
void SigmoidCrossEntropyWeightedLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < num; ++i) {
    for (int ii = 0; ii < dim; ++ii) {
      if (target[i*dim+ii] != -1) {
        loss -= input_data[i*dim+ii] * (target[i*dim+ii] - (input_data[i*dim+ii] >= 0)) -
            log(1 + exp(input_data[i*dim+ii] - 2 * input_data[i*dim+ii] * (input_data[i*dim+ii] >= 0)));
      }
    }
  }
