#ifndef CAFFE_SIGMOID_CROSS_ENTROPY_WEIGHTED_LOSS_LAYER_HPP_
#define CAFFE_SIGMOID_CROSS_ENTROPY_WEIGHTED_LOSS_LAYER_HPP_
...
/**
 * @brief Computes the cross-entropy (logistic) loss with weights @f$
 *          E = \frac{-1}{n} \sum\limits_{n=1}^N \left[
 *                  p_n \log \hat{p}_n +
 *                  (1 - p_n) \log(1 - \hat{p}_n)
 *              \right] * \left[ w_n != -1 \right]
 *        @f$, often used for predicting targets interpreted as probabilities.
 *        The weights indicate whether the n-th example should be ignored. 
 *
 ...
 *   -# @f$ (N \times C \times H \times W) @f$
 *      the targets @f$ y \in [0, 1] or -1 (if to be ignored) @f$
...
class SigmoidCrossEntropyWeightedLossLayer : public LossLayer<Dtype> {
 public:
  explicit SigmoidCrossEntropyWeightedLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "SigmoidCrossEntropyWeightedLoss"; }
...
#endif  // CAFFE_SIGMOID_CROSS_ENTROPY_WEIGHTED_LOSS_LAYER_HPP_
